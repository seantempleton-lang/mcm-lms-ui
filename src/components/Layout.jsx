  ...
  return (
    <>
      <div className="topbar"><div className="topbar-inner">
        ...
        <div className="nav">
          <NavLink className={({isActive})=>`chip ${isActive?'active':''}`} to="/">Dashboard</NavLink>
          {(me?.role==='SUPERVISOR' || me?.role==='ADMIN') && (<>
            <NavLink className={({isActive})=>`chip ${isActive?'active':''}`} to="/supervisor/sessions">Sessions</NavLink>
            <NavLink className={({isActive})=>`chip ${isActive?'active':''}`} to="/supervisor/competencies">Competencies</NavLink>
          </>)}
