...
function NavItem({ to, icon, children, onNavigate }) {
  return (
    <NavLink className={({ isActive }) => `side-link ${isActive ? 'active' : ''}`} to={to} title={children} onClick={onNavigate}>
      {icon}
  ...
  return (
    <>
      ...
      <div className={`app-shell rail-shell ${mobileExpanded ? 'sidebar-expanded' : ''}`}>
        {isMobile && mobileExpanded && <button className="sidebar-backdrop" onClick={() => setSidebarOpen(false)} aria-label="Close navigation" />}

        <aside className={`sidebar rail ${isMobile ? 'mobile' : 'desktop'} ${mobileExpanded ? 'expanded' : ''}`}>
          {isMobile && (
            <button className="rail-handle" onClick={() => setSidebarOpen((current) => !current)} aria-label="Toggle navigation">
