...
export default function MatrixView() {
  ...
  const awardedCells = users.reduce((sum, u) =>
    sum + competencies.filter(c => u.awards[c.code] !== null).length, 0);
    ...
    <div className="grid" onClick={() => setTooltip(null)}>
      ...
      <div className="card" style={{ overflowX: 'auto' }}>
        <table style={{ borderCollapse: 'collapse', width: '100%', minWidth: 600 }}>
          <thead>
            <tr>
              <th style={{ padding: '8px 12px', textAlign: 'left',
                ...
                fontSize: '0.85rem', minWidth: 160 }}>
                Person
              </th>
              {competencies.map(c => (
                <th key={c.id} style={{ padding: '8px 10px', textAlign: 'center',
                  borderBottom: '1px solid var(--line)', color: 'var(--muted)',
                  fontSize: '0.8rem', minWidth: 100 }}>
                  <div style={{ fontWeight: 700, color: 'var(--brand)' }}>{c.code}</div>
          ...
          <tbody>
            {users.map(user => {
              const awarded = competencies.filter(c => user.awards[c.code]).length;
              return (
                <tr key={user.id} style={{ borderBottom: '1px solid var(--line)' }}>
                  ...
                  {competencies.map(c => {
                    const award = user.awards[c.code];
                    return (
                      <td key={c.id} style={{ padding: '8px 10px', textAlign: 'center' }}>
                        ...
                        >
                          {award
                            ? <span style={{ color: 'var(--good)', fontSize: '1rem' }}>✓</span>
      ...
      {tooltip && (
        <div onClick={e => e.stopPropagation()} style={{
          ...
          <button className="btn ghost" style={{ width: '100%', marginTop: 12, padding: '6px' }}
